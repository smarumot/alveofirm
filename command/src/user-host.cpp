...
int main(int argc, char** argv) {
    ...
    int n[DATA_SIZE];
    n[0]=0;
    int LUT_flag = 0b10000000000000000000;
    int LUT_address = 0;
    int LUT_flag_rd = 0;
    for (int i = 0; i < DATA_SIZE; ++i) {
    	...
    	LUT_address = 32*i;
        bo1_map[i] = LUT_flag + LUT_address + LUT_flag_rd + n[i];//read_data[i];
    	//bo0_map[i] = i;
    	bo0_map[i] = i;
        bo2_map[i] = 0;
        n[i+1]=n[i]+1;
        //bufReference[i] = bo0_map[i] + bo1_map[i]; //Generate check data for validation
    if(i<100){std::cout<<" EVENT_num "<<i<<" input_A "<<bo0_map[i]<<" input_B "<<bo1_map[i]<<std::endl;}
    }
    ...
    for(int i=0; i<100;i++){
	  ...
	  output_num++;
    		std::cout<<" A "<< bo0_map[i]<<" B "<<bo1_map[i]<<" C "<<bo2_map[i]<<" [input A] "<<bo0_map[i-3721]<<std::endl;
    	}
    }
    if(output_num==0)cout<<"アウトプットイベントなし"<<endl;
    */
